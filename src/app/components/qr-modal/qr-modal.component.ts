...
export class QrModalComponent implements OnInit {
  ...
  nano_scheme = /^(kiz|kizn|kiznrep|kiznseed|kiznkey):.+$/g;
  ...
  onCodeResult(resultString: string) {
    ...
    if (this.util.account.isValidAccount(resultString)) {
      ...
      if (['kizn:', 'kiznrep:', 'kiz:'].includes(url.protocol) && this.util.account.isValidAccount(url.pathname)) {
        type = 'account';
      } else if (['kiznseed:', 'kiznkey:'].includes(url.protocol) && this.util.nano.isValidHash(url.pathname)) {
        type = 'hash';
      }
    } else {
      type = 'generic';
